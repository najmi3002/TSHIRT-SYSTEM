        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16">
                ...
                <div class="flex items-center justify-center bg-white rounded-lg shadow-lg p-6">
                    <img src="{{ asset('storage/'.$product->image_path) }}" alt="{{ $product->name }}" class="object-contain max-h-96 w-full">
                </div>

                <!-- Product Details and Form -->
                <div class="bg-white rounded-lg shadow-lg p-8">
                    <h1 class="text-3xl font-bold text-gray-900">{{ $product->name }}</h1>
                    <p class="mt-4 text-gray-600">{{ $product->description ?? 'A high-quality t-shirt with a clean and bold look.' }}</p>
                    <p class="text-3xl font-light text-gray-900 mt-6">RM{{ number_format($product->price, 2) }}</p>
