                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="GET" action="" class="flex flex-wrap gap-4 items-end">
                        <div>
                            ...
                            <input type="text" name="product_id" id="product_id" value="{{ request('product_id') }}" class="mt-1 block w-32 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200">
                        </div>
                        <div>
                            <label for="product_name" class="block text-xs font-medium text-gray-700">Product Name</label>
                            <input type="text" name="product_name" id="product_name" value="{{ request('product_name') }}" class="mt-1 block w-48 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200">
                        </div>
                        <div>
                            ...
                            <a href="{{ url()->current() }}" class="ml-2 px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300 transition">Reset</a>
                        </div>
                    </form>
                </div>
            </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($products as $product)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-bold">{{ $product->id }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <img src="{{ asset('storage/'.$product->image_path) }}" alt="Product Image" class="h-16 w-16 object-cover rounded-md shadow-sm">
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-semibold text-gray-900">{{ $product->name }}</div>
                                        <div class="text-sm text-gray-500">RM{{ number_format($product->price, 2) }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex flex-wrap gap-2">
                                            @if(is_array($product->collar_type))
                                                @foreach($product->collar_type as $type)
                                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                                        {{ $type['name'] }} (+{{ number_format($type['addon_price'], 2) }})
                                                    </span>
                                                @endforeach
                                            @endif
                                            @if(is_array($product->fabric_type))
                                                @foreach($product->fabric_type as $type)
                                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                                        {{ $type['name'] }} (+{{ number_format($type['addon_price'], 2) }})
                                                    </span>
                                                @endforeach
                                            @endif
                                            @if(is_array($product->sleeve_type))
                                                @foreach($product->sleeve_type as $type)
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end items-center space-x-3">
                                            <button @click="openModal(false, {{ json_encode($product) }})" class="text-indigo-600 hover:text-indigo-900 font-semibold">Edit</button>
                                            <form action="{{ url('/admin/products/'.$product->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this product?');" class="inline">
                                            @csrf
                                            @method('DELETE')
    ...
    <script>
        function productManager() {
            return {
                showModal: false,
                isNewProduct: true,
                formAction: '{{ url('/admin/products') }}',
                product: {},
                
                openModal(isNew, productData = null) {
                    this.isNewProduct = isNew;
                    if (isNew) {
                        this.formAction = '{{ url('/admin/products') }}';
                        this.product = {
                            name: '',
                            price: '',
                        ...
                        this.formAction = '{{ url('/admin/products') }}/' + productData.id;
